...
class sessionmgr(Component[apiclient]):
    ...
    async def _bililogin(self):
        ...
        self._sdkaccount = {"uid": uid, "access_key": access_key}
        ...
        for _ in range(5):
            try:
                ...
                if resp.quest_list and any(
                    ...
                    if quest.quest_id == 11008001
                    and quest.result_type == eMissionStatusType.AlreadyReceive
                ):  # clear normal 8-1 and unlock daily task
    ...
    async def request(
        self, request: Request[TResponse], next: RequestHandler
    ) -> TResponse:
        if not self._logged:

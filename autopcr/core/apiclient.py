...
class apiclient(Container["apiclient"]):
    ...
    def _no_null_key(obj):
        if type(obj) == dict:
            ...
            return {
                k: apiclient._no_null_key(v) for k, v in obj.items() if k is not None
            }
        elif type(obj) == list:
    ...
    async def _request_internal(self, request: Request[TResponse]) -> TResponse:
        if not request:
            return None
        logger.info(
            f"{self.user_name} requested {request.__class__.__name__} at /{request.url}"
        )
        key = apiclient._createkey()
        request.viewer_id = (
            b64encode(
                apiclient._encrypt(str(self.viewer_id).encode("utf8"), key)
            ).decode("ascii")
            if request.crypted
        ...
        try:
            ...
            response0 = (
                apiclient._unpack(response0)[0] if request.crypted else loads(response0)
            )
        except:
            raise NetworkException
        ...
        if DEBUG_LOG:
            with open("req.log", "a") as fp:
                ...
                fp.write(
                    json.dumps(
                        json.loads(request.json(by_alias=True)),
                        indent=4,
                        ensure_ascii=False,
                    )
                    + "\n"
                ...
                fp.write(
                    json.dumps(dict(resp.headers), indent=4, ensure_ascii=False) + "\n"
                )
        ...
        if (
            r"source_ini/get_maintenance_status?format=json" == request.url
            and "store_url" in response0["data_headers"]
        ):
            match = search(
                r"(?<=gzlj_)(\d+\.\d+\.\d+)", response0["data_headers"]["store_url"]
            )
            if not match:
            ...
            if "维护" in response.data.server_error.message:
                try:
                    response.data.server_error.message = (
                        response.data.maintenance_message
                    )
                except:
                    pass
